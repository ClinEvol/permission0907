...
public class MyFileUtils {
    ...
    private static String absolutePath=null;//为当前项目下的webapp/upload

    static {
        File file=new File("");//如不写路径就会在当前项目下新建
        absolutePath = file.getAbsolutePath()+"/src/main/webapp/upload/";
    }
    ...
    public static ResponseEntity<byte[]> download(File file){
        ...
        String newName = null;
        try {
            newName = new String(file.getName().getBytes("UTF-8"),"ISO-8859-1");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        //设置浏览器的弹框
